...
class Point < GeometryValue
  ...
  def intersectPoint point
   if real_close_point(self.x,self.y, point.x,point.y) then
    self
   else
    NoPoints.new
   end
  ...
  def intersectLine line
    if real_close(self.y, line.m * self.x + line.b) then 
      self
    else
      NoPoints.new
    end
  ...
  def intersectVerticalLine vline 
    if real_close(self.x,vline.x) then
      self
    else
      NoPoints.new
    end
...
class Line < GeometryValue
  ...
  def intersectPoint point
    if real_close(point.y, self.m * point.x + self.b) then 
      point
    else
      NoPoints.new
    end
...
class Let < GeometryExpression
  ...
  def eval_prog env
    arr    = []
    arr[0] = @s 
    arr[1] = @e1.eval_prog(env)
    env =[arr] + env
    @e2.eval_prog(env)
  end
  def shift(dx,dy)
    self.eval_prog(env).shift(dx,dy)
  end
end
